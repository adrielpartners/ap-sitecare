...
# Decision 042: Use Hostinger SSH/SFTP and Dropbox offline OAuth for SiteCare Pro portable backups

## Decision

SiteCare Pro long-term backups are monthly, full website-files-and-database,
portable packages retained for 24 months at one independent off-site
destination. Hostinger routine backups and restoration remain separate.

The first source adapter for Hostinger Agency shared Cloud hosting uses durable
SSH-key authentication and SFTP. It downloads the readable WordPress tree and
runs a fixed WP-CLI database export over SSH, avoiding routine password entry
and allowing the plugin-reported database password to be retired after the
connection passes live validation. Legacy Local VPS paths are quarantined
unless an explicit worker mount makes them real.

Dropbox is the first destination. Dashboard-managed Dropbox connections use
the authorization-code flow with offline access, encrypt the refresh token,
and renew short-lived access automatically. Static runtime access tokens remain
temporarily compatible for deployment cutover.

Each backup is stored beneath a stable client folder and `YYYY/MM/backup-id`.
Every filename contains the canonical hostname, UTC timestamp, and backup ID.
The package contains standard tar/gzip files, compressed SQL, manifest,
SHA-256 checksums, and a restoration README. Exact remote object paths are
retained when destination settings or client display names change.

Backup creation and credential changes are transactional and audited. Success
and failure email is generated by the Dashboard notification system for every
configured backup-category recipient. Retention initially performs scheduled,
audited dry runs; remote deletion needs a separate explicit enablement after
production evidence is reviewed.

Restoration is supervised: SiteCare verifies evidence, creates temporary
downloads and a technician checklist, and records target, notes, timestamps,
and outcome. It does not execute unattended restoration.

## Rationale

Hostinger's shared-hosting API does not expose a reliable full-file export,
while Agency hosting documents durable SSH/SFTP key access. Dropbox offline
OAuth satisfies the set-it-and-forget-it requirement without relying on an
indefinitely valid bearer access token. Portable artifacts avoid locking
restoration to SiteCare or Hostinger.

## Tradeoffs

- Each live Hostinger account boundary and WP-CLI availability must be proven
  before a site can count as backup-ready.
- SFTP temporarily consumes local disk equal to the readable source plus the
  generated archives.
- A revoked Dropbox grant requires an explicit reconnect, although ordinary
  access-token expiry is automatic.
