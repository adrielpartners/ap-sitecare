...
<template>
  <div>
    ...
    <div v-if="health" class="stack">
      ...
      <AppPanel title="Database schema"><p>Latest migration: <strong>{{ health.database.migration?.id }} · {{ health.database.migration?.name }}</strong></p><p class="text-meta">Applied {{ health.database.migration ? formatSiteCareDateTime(health.database.migration.applied_at) : 'unknown' }}</p></AppPanel>
    </div>
  </div>
</template>
